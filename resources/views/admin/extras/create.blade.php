<x-app-layout>
    <x-slot:title>Nuevo Extra | AutoAlquiler</x-slot:title>
    ...
    <div class="max-w-md mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <h1 class="text-xl font-semibold text-[var(--foreground)] mb-6">Nuevo Extra</h1>
            ...
            <form action="{{ route('admin.extras.store') }}" method="POST" class="space-y-4">
                ...
                <div>
                    ...
                    <select id="selection_type" name="selection_type"
                        ...
                        <option value="single"   {{ old('selection_type', 'single') === 'single'   ? 'selected' : '' }}>Selección única (checkbox)</option>
                        <option value="multiple" {{ old('selection_type') === 'multiple' ? 'selected' : '' }}>Cantidad (número de unidades)</option>
                    </select>
                ...
                <div class="flex justify-end gap-2 pt-2">
                    ...
                    <x-btn type="submit" size="sm">Guardar extra</x-btn>
                </div>
            </form>
        </div>
