<x-app-layout>
    <x-slot:title>Editar Categoría | AutoAlquiler</x-slot:title>
    ...
    <div class="max-w-xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <h1 class="text-xl font-semibold text-[var(--foreground)] mb-6">Editar Categoría</h1>
            ...
            <form action="{{ route('admin.categorias.update', $category) }}" method="POST" class="space-y-4">
                ...
                <div>
                    ...
                    <textarea id="description" name="description" rows="3"
                              ...
                              placeholder="Describe brevemente esta categoría de vehículos...">{{ old('description', $category->description) }}</textarea>
                    @error('description')
                        <p class="text-[var(--danger)] text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="flex justify-end gap-2 pt-2">
                    ...
                    <x-btn type="submit" size="sm">Actualizar categoría</x-btn>
                </div>
            </form>
        </div>
